...
Examples of such criteria are following the object term of triples having a predicate term with a specific \gls{iri} or the criterion \texttt{Cmatch}~\cite{hartig2016walking} which consist of dereferencing
\gls{iri} related to the variable of the query.
For each triple in the knowledge base each reachability criterion determines which new \glspl{iri} to follow.
Concurrently, the query is executed in the internal knowledge base and returns results in a streaming manner, so the traversal does not need to finish before the user begins receiving output.
