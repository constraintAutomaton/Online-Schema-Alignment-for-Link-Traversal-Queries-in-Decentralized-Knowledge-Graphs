...
In other words, these rules apply only within the designated set of \gls{iri}.
...
To discover alignment rules, we modify the traversal policy to follow object terms of triples with predicate \gls{iri} \texttt{semmap:ruleSetLocation}, where the dereferenced term provides a rule set.
When the engine encounters a rule set, it applies the rules to its internal \gls{kg} with consideration to the subweb.
The resulting \gls{kg} is used for both query joins and the traversal policy.
