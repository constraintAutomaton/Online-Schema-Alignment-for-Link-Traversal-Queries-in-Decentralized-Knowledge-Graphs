...
At a step $s$ a query engine receive a new \glspl{kg} $KG$ in the form of a stream.
This $KG$

A query engine receive a  $T$ at

We define $AR_s$ as the set of mapping between subwebs $S$ and schema algiment rules $R$ at the step $s$.
\begin{equation} \label{eq:schema_alignment_scoped}
    AR_s = \left\{ S_i \mapsto R_i| 1 \ge i \le n \right\} \cup \left\{ S_j \mapsto R_j| 1 \ge j \le n_s \right\}
\end{equation}
Where $n$ is the number of query issuer defined alignment rules and $n_t$ is the number of discovered alignment rules at the step $s$.
