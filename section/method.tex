...
At a step $s$, the query engine receives a new \gls{kg}, denoted as $G$.
...
The alignment process is then executed over $G$, producing an aligned graph $G^{\prime}$.
...
It is crucial to use the triples of $G^{\prime}$ rather than those of $G$, since the reachability criteria are not designed to handle alignments across different vocabularies.
