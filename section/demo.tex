...
\iffalse
The modifications include: alternative version of the vocabulary,
HTTPS variant of the vocabulary,
\gls{iri} derived from literal values, e.g., ``male'', ``female'', ``Firefox'', ``Chrome'' and
alignment between certain \glspl{iri} from the original vocabulary and the corresponding terms in Wikidata and DBpedia.
...
%The demonstrator can also be deployed using a Docker Compose~\footnote{https://docs.docker.com/compose/} setup, which automatically generates the dataset, instantiates the servers, and launches the web application.
Queries can be run over different network configurations the base network or the network with alternative vocabularies.
...
%We also provide a visual representation of the base network's data model.
\begin{figure}
    \centering
    \includegraphics[width=0.45\linewidth]{img/demo_screenshot.png}
